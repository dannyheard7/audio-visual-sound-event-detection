...
class FileFormat(object):
		...
		try:
			...
			with open(self.filepath) as filename:
				for line in filename:
					...
					if audioFile not in list(self.labelsDict.keys()):
						#does not exist
	...
	def validatePredictedDS(self, predictedDS):
		...
		#check bothways
		for audioFile in list(predictedDS.labelsDict.keys()):
			if(audioFile not in list(self.labelsDict.keys())):
				return False


		for audioFile in list(self.labelsDict.keys()):
			if(audioFile not in list(predictedDS.labelsDict.keys())):
				return False
	...
	def computeMetrics(self, predictedDS, output_filepath):
		...
		with open(output_filepath, "a") as Metric_File:
			...
			Metric_File.write("Number of Audio Files = " + str(len(list(self.labelsDict.keys()))))
		Metric_File.close()
